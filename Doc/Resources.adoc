...
browser. +
On GitHub, `.adoc` files are rendered automatically.
====
...
RISC-V CPU" tutorial.  We recommend that you practice all these
download/installation actions as soon as you can (how about right
now!) just to make sure it all works for you.  Closer to the tutorial
date, please refresh everything/rebuild so you have the latest
versions of everything.

The following figure is taken from the tutorial slide deck, showing
the tutorial plan:
...
1. `git clone` a repo for *Fife* and *Drum* code (Section
   <<sec_book_repo>>), textbook, etc.
+
This enables you to read all *Fife* and *Drum* hardware design code.

2. Download and install the *bsc* compiler (Section <<sec_bsc>>).
+
This enables you to compile and run Bluesim simulations of *Fife* and *Drum*.

3. `git clone` and install *TestRIG* (Section <<sec_testrig>>).
+
This enables using *TestRIG* on *Fife* and *Drum* using Bluesim simulation.

4. `git clone` and install *Verilator* (Section
   <<sec_verilator>>).
+
This enables you to compile and run Verilog simulations of *Fife* and *Drum*, and *TestRIG*.

// ****************************************************************
[#sec_book_repo]
== *Fife* and *Drum* hardware design source code, textbook, and more
...
This tool is sufficient for compiling and building the *Fife* and *Drum*
...
This section is needed if you plan use Verilog simulation for *Fife*
and *Drum*, and can be skipped otherwise.
...
* You wish to plug *Fife* or *Drum* Verilog into your own Verilog SoC.
* You wish to go further with *Drum* or *Fife* Verilog into an FPGA or ASIC flow.
* You may wish to take *Fife* or *Drum* Verilog through other
...
the *Fife* and *Drum* CPUs, and different Verilog simulators may use
...
Please `git clone` the *TestRIG* repository:
https://github.com/CTSRD-CHERI/TestRIG[^]

Remember to perform this recursive submodule initialization (this
brings in several other respositories that *TestRIG* depends on):

```
    $ cd <your clone of TestRIG repo>
...
on *Fife* and *Drum* and *Sail*, and to verify the responses. *QCVengine* is
...
// ================================================================
=== Official *RISC-V Formal Specification* (written in *Sail*)
...
(simulation).  For "B" we use *Fife* or *Drum* in a wrapper for
*TestRIG*.
...
ISA.  The *Sail* compiler is written in *OCaml*, and is built as part
of installing the *OCaml* system.  Thus, to build the *Sail* RISC-V
simulator, one needs to install the *OCaml* system which also installs
the *Sail* compiler.

// ----------------------------------------------------------------
==== Installing the *OCaml* system (including the *Sail* compiler)
...
install the rest of the *OCaml* system (which includes the *Sail*
compiler).
...
Once installed, the OCaml environment is usually set up like this:
```
    $ eval $(opam config env)
```

which will include the *Sail* compiler in your PATH, which you can
confirm like this:

```
    $ which sail
    <your-home-directory>/.opam/default/bin/sail
```

// ----------------------------------------------------------------
==== Building *Sail RISC-V Formal Specification* simulator with *Sail* compiler

[NOTE]
====
You can practice building the simulator now, using the following command.

However, we'll need to edit the *Sail* code to configure it to support
the same RISC-V configuration choices as *Fife* and *Drum*.  We will
provide a patch for this edit closer to the date of the tutorial.

Before the tutorial, you will need to apply the provided patch, and rebuild.
====
